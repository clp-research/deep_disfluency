...
#from feature_extraction.swbd_MSalign_to_SWDA_words import start_times
sys.path.append("../")
from utils.tools import convertFromEvalTagsToIncDisfluencyTags
import argparse
...
def fill_in_time_approximations(word_timing_tuples, idx):
    """Called when the start and end time of a word are the same (i.e. it has no length, due to technical error), it searches forward in the list until 
    it meets a word with a different end time, then spreads the time of the affected timing tuples stand and end time accordingly.
    ...
    assert start_time == end_time,"Function not needed- different start and end times!"
    assert len(word_timing_tuples)>1
    if end_time == word_timing_tuples[-1][2]: #this end time is the same as the latest one, need backward search for time
    ...
    assert total_time > 0.0,str(word_timing_tuples[affected_indices[0]:]) + str(idx)
    mean_len = total_time/len(affected_indices)
    for i in range(idx, idx + len(affected_indices)):
        end_time = start_time + mean_len
        word_timing_tuples[i] = (word_timing_tuples[i][0],start_time, end_time)
        start_time = end_time
