...
def convert_to_simple_label(tag,rep="1_trp"):
    """Takes the complex tag set and gives back the simple, smaller version with ten tags:
    """
    disftag = "<f/>"
    if "<rm-" in tag: disftag = "<rm-0/>"
    elif "<e" in tag: disftag = "<e/>"
    if "trp" in rep: #if combined task with TTO
        TTO_tag = tag[tag.rfind("><")+1:]
        return disftag+TTO_tag
