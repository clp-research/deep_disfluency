...
def final_hyp_from_increco_and_incremental_metrics(increco, gold,
    ...
    for n_timing, n_words, n_tags in zip(increco[0], increco[1], increco[2]):
...
def get_tag_data_from_corpus_file(f, representation="1", limit=8):
    ...
    for line in f:
        ...
        if not len(spl) == 6:
            continue
        ref, start_time, end_time, word, postag, disftag = spl
...
def final_output_accuracy_interval_level(hyp, reference, tag_dict,
    ...
    r = [(float(a[0]), float(a[1]), d) for a, b, c, d in
...
def getRepairsWithStartWordIndex(tags, words, rp_start_index, gold_tags=None,
    ...
    if save_context:
        for id in repairDict.keys():
            ...
            repairDict[id].context = ["{0}|{1}".format(x, y)
                                      for x, y in
            ...
            if gold_tags:
                ...
                repairDict[id].gold_context = ["{0}|{1}".format(x, y)
                                               for x, y in
                                               zip(word_context, tag_context)]
    ...
    for c in range(rp_start_index-1, -1, -1):
        ...
        for tag in get_tags(tags[c]):
            for id in repairDict.keys():
                if repairDict[id].reparandumComplete:
                    continue
                allComplete = False
                if '<i id="{}"/>'.format(id) in tag:
                    repairDict[id].interregnumWords.insert(0, words[c])
                if '<rm id="{}"/>'.format(id) in tag:
                    repairDict[id].reparandumWords.insert(0, words[c])
                if '<rms id="{}"/>'.format(id) in tag:
                    repairDict[id].reparandumWords.insert(0, words[c])
                    repairDict[id].repararandumComplete = True
        if allComplete:
            break
    ...
    for c in range(rp_start_index, len(tags)):
        ...
        for id in repairDict.keys():
            wordAdded = False
            for tag in get_tags(tags[c]):
                if repairDict[id].repairComplete:
                    continue
                allComplete = False
                if re.match('<rp[s] id="{}"/>'.format(id), tag):
                    if not wordAdded:
                        repairDict[id].repairWords.append(words[c])
                    wordAdded = True
                if re.match('<rpn[repdelsub]* id="{}"/>'.format(id), tag):
                    if not wordAdded:
                        repairDict[id].repairWords.append(words[c])
                    wordAdded = True
                    repairDict[id].repairComplete = True
                    repairDict[id].type = tag[4:7]
        if allComplete:
            break
