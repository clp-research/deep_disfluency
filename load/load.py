...
from rnn_disf_detection.utils.tools import convertFromEvalTagsToIncDisfluencyTags, add_word_continuation_tags, contextwinbackwards,\
 indicesFromLength, convert_to_simple_idx, convert_to_simple_label, convert_from_full_tag_set_to_idx
from rnn_disf_detection.feature_extraction.feature_utils import fill_in_time_approximations
...
def load_data_from_array(data, n_acoust, cs=2, bs=9, tags="1_trp_simple", full_idx_to_label_dict=None, idx_to_label=None):
    ...
    if not tags == "1_trp_simple": #TODO this is a hack as the data should be in the right format remove after exps.
        for i in range(0,len(labels)):
            #if 'simple' in tags:
            labels[i] = convert_from_full_tag_set_to_idx(full_idx_to_label_dict[labels[i]],tags,idx_to_label)
